<?php
	...
	$sql_timkiem="SELECT * FROM tbl_sanpham as a, tbl_danhmuc as b WHERE a.id_danhmuc=b.id_danhmuc and a.tensanpham 
					Like '%".$value."%' ORDER BY a.id_sanpham DESC  LIMIT $begin,4 ";
	$query_timkiem=mysqli_query($mysqli,$sql_timkiem);

	$query_phantrang=mysqli_query($mysqli,"SELECT * FROM tbl_sanpham as a, tbl_danhmuc as b WHERE a.id_danhmuc=b.id_danhmuc and a.tensanpham 
	Like '%".$value."%'");
				...
				<div class="col-md-10">
					...
					<div>
						...
						<ul class="list_trang">
							...
							?>
							<li <?php if($i==$trang){echo 'Style="background:red"';}?>><a href="?quanly=timkiem&tukhoa=<?php echo $value?>&trang=<?php echo $i?>"><span><?php echo $i?></span></a></li>
							<?php 
							}
							?>
						</ul>
					...
					<?php
							...
							echo '<h2 style="text-align: center;padding: 2% 0;">Không có sản phẩm với tên tìm kiếm</h2>';
						}
					?>
				</div>
			</div>
